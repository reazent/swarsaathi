...
def generate_song_audio(
    ...
    *,
    ...
    # Explicit instrumental → no vocals.
    if instrumental:
        body = {
            "prompt": prompt.strip(),
            "instrumental": True,
    ...
    # User-supplied lyrics → style/mood in `tags`, song text in `lyrics`.
    if lyric_text:
        body = {
    ...
    # No lyrics provided → prompt-to-audio invents sung lyrics (instrumental=False).
    body = {
        "prompt": prompt.strip(),
        "instrumental": False,
        "duration": float(duration),
